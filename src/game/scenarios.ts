  ...
  const base: import('./types').GaugeConfig[] = [
    {
      ...
      driftRate: 0.30, noiseAmplitude: 1.0, decayRate: 0.35,
      valveReduction: 14, valveCooldown: 3500,
    },
    {
      id: 'n2', label: 'N₂', unit: '%',
      baseline: 38, initialValue: 40,
      greenMax: 60, amberMax: 80,
      driftRate: 0.25, noiseAmplitude: 0.8, decayRate: 0.30,
      valveReduction: 14, valveCooldown: 3500,
    },
    {
      id: 'w2', label: 'W₂', unit: '%',
      ...
      greenMax: 60, amberMax: 80,
      driftRate: 0.20, noiseAmplitude: 0.7, decayRate: 0.25,
      valveReduction: 14, valveCooldown: 3500,
    },
  ];
  if (overrides) {
...
export const scenarios: ScenarioConfig[] = [
  /* ── A: Chemical Reactor Control ── */
  {
    ...
    gauges: defaultGauges({
      co2: { initialValue: 48, driftRate: 0.40 },
      n2: { initialValue: 45, driftRate: 0.35 },
      w2: { initialValue: 42, driftRate: 0.28 },
    }),
    output: {
      label: 'Reactor Output',
      baseline: 50, initialValue: 72,
      threshold: 80, safeMax: 75,
      driftRate: 0.40, decayRate: 0.5,
      noiseAmplitude: 0.6,
    },
  ...
  /* ── B: Gas Mixture Balance ── */
  {
    ...
    timeLimit: 75,
    gauges: defaultGauges({
      co2: { initialValue: 50, driftRate: 0.55, noiseAmplitude: 1.2 },
      n2: { initialValue: 45, driftRate: 0.40, noiseAmplitude: 1.0 },
      w2: { initialValue: 40, driftRate: 0.30, noiseAmplitude: 0.8 },
    }),
    output: {
      label: 'Process Output',
      baseline: 50, initialValue: 50,
      threshold: 85, safeMax: 80,
      driftRate: 0.15, decayRate: 0.25,
      noiseAmplitude: 0.7,
    },
    initialSystemOn: true,
    initialAlarmActive: false,
  ...
  /* ── C: Alarm Response ── */
  {
    ...
    timeLimit: 50,
    gauges: defaultGauges({
      co2: { initialValue: 72, driftRate: 0.20, decayRate: 0.08 },
      n2: { initialValue: 55, driftRate: 0.18 },
      w2: { initialValue: 48, driftRate: 0.14 },
    }),
    output: {
      label: 'Process Output',
      baseline: 50, initialValue: 60,
      threshold: 85, safeMax: 80,
      driftRate: 0.10, decayRate: 0.20,
      noiseAmplitude: 0.5,
    },
  ...
  /* ── D: Sudden Surge ── */
  {
    ...
    timeLimit: 60,
    gauges: defaultGauges({
      co2: { initialValue: 38, driftRate: 0.18 },
      n2: { initialValue: 36, driftRate: 0.14 },
      w2: { initialValue: 34, driftRate: 0.12 },
    }),
    output: {
      label: 'Reactor Output',
      baseline: 50, initialValue: 48,
      threshold: 85, safeMax: 80,
      driftRate: 0.12, decayRate: 0.35,
      noiseAmplitude: 0.5,
    },
    initialSystemOn: true,
    initialAlarmActive: false,
